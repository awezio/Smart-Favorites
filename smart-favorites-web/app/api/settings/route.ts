...
import { isAnyProviderConfigured } from "@/lib/ai/ai-configured";
import {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      aiConfigured: isAnyProviderConfigured(providerStatus),
      ...
      autoTitleEnabled: userSettings?.auto_title_enabled !== false,
      ragRerankEnabled: userSettings?.rag_rerank_enabled !== false,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (body.auto_title_enabled !== undefined) {
      updateData.auto_title_enabled = Boolean(body.auto_title_enabled);
    }
    if (body.rag_rerank_enabled !== undefined) {
      updateData.rag_rerank_enabled = Boolean(body.rag_rerank_enabled);
    }
    if (body.title_llm_provider !== undefined) {
      updateData.title_llm_provider =
        typeof body.title_llm_provider === "string" && isSupportedProvider(body.title_llm_provider)
          ? body.title_llm_provider
          : null;
    }
    if (body.title_llm_model !== undefined) {
      updateData.title_llm_model =
        typeof body.title_llm_model === "string" && body.title_llm_model.trim()
          ? body.title_llm_model.trim()
          : null;
    }
